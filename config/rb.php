    ...
    class OODBBean implements \IteratorAggregate, \ArrayAccess, \Countable, Jsonable
    {
        ...
        private function getOwnList($type, $redbean)
        {
            ...
            if ($this->getID()) {
                ...
                if (!is_numeric($firstKey) || $firstKey === null) {
                    ...
                    $beans = $redbean->find($type, array(), " {$joinSql} $myFieldLink = :slot0 ".$this->withSql,
                      $bindings);
                } else {
                ...
                if (isset($this->__info["sys.parentcache.$property"])) {
                    ...
                    if (!is_null($this->properties[$fieldLink])) {
                        ...
                        if ($bean->id !== $this->properties[$fieldLink] && self::$autoResolve) {
                            $type = $this->beanHelper->getToolbox()->getWriter()->inferFetchType($this->__info['type'],
                              $property);
                            if (!is_null($type)) {
                                $bean = $redbean->load($type, $this->properties[$fieldLink]);
        ...
        public function __set($property, $value)
        {
            ...
            $isFieldLink = (($pos = strrpos($property,
                  '_id')) !== false) && array_key_exists(($fieldName = substr($property, 0, $pos)), $this->properties);
        ...
        public function countOwn($type)
        {
            ...
            if ($this->getID()) {
                ...
                if (!is_numeric($firstKey) || $firstKey === null) {
                    $bindings = $this->withParams;
                    $bindings[':slot0'] = $this->getID();
                    $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, array(),
                      " {$joinSql} $myFieldLink = :slot0 ".$this->withSql, $bindings);
                } else {
                    $bindings = array_merge(array($this->getID()), $this->withParams);
                    $count = $this->beanHelper->getToolbox()->getWriter()->queryRecordCount($type, array(),
                      " {$joinSql} $myFieldLink = ? ".$this->withSql, $bindings);
                }
